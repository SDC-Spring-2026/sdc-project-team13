...
export default async function AccountPage() {
  ...
  try {
    ...
    if (conn.driver === "postgres") {
      ...
      const row = conn.db
        .prepare(`SELECT github FROM ${tbl("members")} WHERE discord = ? LIMIT 1`)
        ...
        <div className="space-y-4">
          <Card className="animate-in fade-in-0 slide-in-from-bottom-2 duration-500">
            <CardHeader>
              ...
              <CardDescription>
                Discord: <span className="font-medium text-foreground">{displayName ?? "Signed in"}</span>
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 text-sm text-muted-foreground">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {flags.isAdmin ? <Badge className="gap-1"><Shield className="h-3 w-3" />Admin</Badge> : null}
                {flags.isPresident ? <Badge variant="secondary" className="gap-1"><Shield className="h-3 w-3" />President</Badge> : null}
              </div>

              <div className="rounded-lg border bg-muted/20 p-3">
                <div className="text-xs text-muted-foreground">Linked GitHub</div>
              ...
              <div className="text-xs">
                This page intentionally avoids showing raw IDs or any private credentials.
              </div>
            </CardContent>
          </Card>
        </div>
