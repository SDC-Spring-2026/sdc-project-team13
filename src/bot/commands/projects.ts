...
export async function handleProjects(interaction: ChatInputCommandInteraction) {
    ...
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ text: `${teams.length} project${teams.length !== 1 ? 's' : ''} • Brought to you by Cache 🤖` });

        logger.info(`Projects listed by ${interaction.user.tag} (${teams.length} teams)`);
        await interaction.editReply({ embeds: [embed] });
    } catch (err) {
        logger.error(`Failed to list projects for ${interaction.user.tag}: ${err instanceof Error ? err.message : String(err)}`);
