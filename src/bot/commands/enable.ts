...
export const enableCommand = {
    name: "enable",
    description: "Restore a disabled team channel and unarchive its GitHub repo (admin only)",
    options: [
        {
            type: 3,
            name: "group",
...
export async function handleEnable(interaction: ChatInputCommandInteraction) {
    ...
    if (!isGuildAdmin(interaction)) {
        await interaction.reply({ flags: MessageFlags.Ephemeral, content: "Only server administrators can enable teams." });
        return;
    }
    ...
    if (team.github_repo) {
        await setTeamRepoArchived(team.github_repo, false).catch((e) => {
            logger.error(`Failed to unarchive repo for "${teamSlug}": ${e instanceof Error ? e.message : String(e)}`);
            githubNote = "\n⚠️ GitHub repo unarchival failed — check bot permissions.";
        });
    }

    logger.info(`Team "${teamSlug}" enabled by ${interaction.user.tag}`);
    await interaction.editReply(`✅ **${group}** has been re-enabled. The channel is visible and the repo unarchived.${githubNote}`);
}
