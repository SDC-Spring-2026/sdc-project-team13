...
export async function handleLeave(interaction: ChatInputCommandInteraction) {
  ...
  if (callerPerm === TeamPermissionLevel.LEADER) {
    await interaction.reply({
      flags: MessageFlags.Ephemeral,
      content:
        "You are the leader of this team and cannot leave directly. Transfer leadership first using `/manage transfer`."
    });
    return;
  }
  ...
  try {
    // remove person from group in database
    ...
    logger.info(
      `${interaction.user.tag} (${interaction.user.id}) left "${group}" (team: ${teamSlug})`
    );
    ...
    await interaction.editReply(
      `Failed to leave **${group}**. Please try again or contact an admin.`
    );
  }
}
