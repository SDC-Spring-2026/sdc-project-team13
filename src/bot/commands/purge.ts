...
export const purgeCommand = {
  name: "purge",
  description:
    "Permanently delete a team: channel, role, and GitHub repo (admin only)",
  options: [
    {
      type: 3,
      name: "group",
...
export async function handlePurge(interaction: ChatInputCommandInteraction) {
  ...
  if (!isGuildAdmin(interaction)) {
    await interaction.reply({
      flags: MessageFlags.Ephemeral,
      content: "Only server administrators can purge teams."
    });
    return;
  }
  ...
  if (!team.is_disabled) {
    await interaction.reply({
      flags: MessageFlags.Ephemeral,
      content: `**${group}** must be disabled before it can be purged. Run \`/disable\` first.`
    });
    return;
  }
  ...
  try {
    ...
    await interaction.editReply({
      content: "Purge cancelled (timed out).",
      components: []
    });
    return;
  }
  ...
  if (channel) {
    await channel.delete(`Purged by ${interaction.user.tag}`).catch((e) => {
      logger.error(
        `Failed to delete channel for "${teamSlug}": ${e instanceof Error ? e.message : String(e)}`
      );
  ...
  if (team.role_id) {
    ...
    if (role) {
      await role.delete(`Purged by ${interaction.user.tag}`).catch((e) => {
        logger.error(
          `Failed to delete role for "${teamSlug}": ${e instanceof Error ? e.message : String(e)}`
        );
  ...
  if (team.github_repo) {
    await deleteTeamRepo(team.github_repo).catch((e) => {
      logger.error(
        `Failed to delete repo for "${teamSlug}": ${e instanceof Error ? e.message : String(e)}`
      );
  ...
  await db.tombstoneTeam(teamSlug).catch((e) => {
    logger.error(
      `Failed to tombstone team record for "${teamSlug}": ${e instanceof Error ? e.message : String(e)}`
    );
  ...
  await interaction.editReply({
    content: `✅ **${group}** (${teamSlug}) has been permanently deleted.${suffix}`
  });
}
