...
import { addRepoCollaborator, removeRepoCollaborator } from "../../integrations/githubApp";
...
export const adminCommand = {
  ...
  options: [
    ...
    },
    {
      type: 1, // SUB_COMMAND
      name: "force_remove",
      description: "Force-remove a user from a team (DB + Discord role + GitHub)",
      options: [
        {
          type: 3,
          name: "team",
...
function mustBePrivileged(interaction: ChatInputCommandInteraction): GuildMember | null {
  const member = interaction.member;
  if (!member || typeof member !== "object" || !("roles" in member)) return null;
...
async function ensureTeamsCategory(interaction: ChatInputCommandInteraction) {
  ...
  if (!me.permissions.has(PermissionFlagsBits.ManageChannels)) {
    throw new Error("Missing permission: Manage Channels (cannot create Teams category).");
  }
...
async function getTeamOrReply(interaction: ChatInputCommandInteraction, teamSlug: string) {
...
async function maybeAudit(interaction: ChatInputCommandInteraction, text: string) {
  try {
    const guild = interaction.guild;
    if (!guild) return;
...
export async function handleAdmin(interaction: ChatInputCommandInteraction) {
  ...
  if (group === "audit_log") {
    ...
    if (sub === "set") {
      ...
      await maybeAudit(interaction, `🔧 ${interaction.user.tag} set audit log channel to ${ch}.`);
      return;
    }
    ...
    if (sub === "test") {
      ...
      await ch.send({ content: `✅ Audit log test from ${interaction.user.tag}` });
      await interaction.reply({
        flags: MessageFlags.Ephemeral,
  ...
  if (sub === "setup") {
    ...
    await interaction.editReply(`✅ Teams category is ready: **${category.name}** (${category.id})`);
    await maybeAudit(interaction, `🧰 ${interaction.user.tag} ran /admin setup (Teams category: ${category.id}).`);
    return;
  }
  ...
  if (sub === "diagnostics") {
    ...
    await interaction.reply({
      ...
      content: [
        "**Diagnostics**",
        missing.length ? `Missing env: ${missing.map((m) => `\`${m}\``).join(", ")}` : "Env: OK",
        roleCfgOk
  ...
  if (sub === "repair") {
    ...
    if (!teamSlug) {
      await interaction.editReply("Provide `team`, or run this inside a team channel.");
      return;
    }
    ...
    if (!role) role = guild.roles.cache.find((r) => r.name === teamSlug) ?? null;
    if (!role) {
      ...
      const T = tbl("teams");
      driver()
        .query(`UPDATE ${T} SET role_id = ? WHERE slug = ?`, [role.id, teamSlug])
        .catch(() => {});
    } else if (role.name !== teamSlug) {
      await role.setName(teamSlug, `Repaired role name for ${teamSlug} via /admin repair`).catch(() => {});
    }

    // Channel
    let channel =
      team.channel_id ? guild.channels.cache.get(team.channel_id) : undefined;
    if (!channel || channel.type !== ChannelType.GuildText) {
      ...
      const T = tbl("teams");
      driver()
        .query(`UPDATE ${T} SET channel_id = ? WHERE slug = ?`, [channel.id, teamSlug])
        .catch(() => {});
    } else {
    ...
    for (const m of members) {
      try {
        ...
        if (!gm.roles.cache.has(role.id)) {
          await gm.roles.add(role, `Repaired team role membership for ${teamSlug}`);
          fixed++;
        }
      } catch {
    ...
    await maybeAudit(interaction, `🛠️ ${interaction.user.tag} repaired team ${teamSlug} (synced ${fixed}).`);
    return;
  }
  ...
  if (sub === "force_add") {
    ...
    if (existing !== null) {
      await interaction.editReply(`User is already on **${teamSlug}** in the DB.`);
      return;
    }
    ...
    if (role) {
      ...
      if (gm) await gm.roles.add(role, `Force-added to ${teamSlug} by ${interaction.user.tag}`).catch(() => {});
    }
    ...
    if (memberRow?.github && team.github_repo) {
      await addRepoCollaborator(team.github_repo, memberRow.github).catch((e) =>
        logger.error(`GitHub add collaborator failed: ${e instanceof Error ? e.message : String(e)}`)
      );
    }

    await interaction.editReply(`✅ Added ${user} to **${teamSlug}**.`);
    await maybeAudit(interaction, `➕ ${interaction.user.tag} force-added ${user.tag} to ${teamSlug}.`);
    return;
  }
  ...
  if (sub === "force_remove") {
    ...
    if (role) {
      ...
      if (gm && gm.roles.cache.has(role.id)) {
        await gm.roles.remove(role, `Force-removed from ${teamSlug} by ${interaction.user.tag}`).catch(() => {});
      }
    }

    const memberRow = await db.getMember(user.id);
    if (memberRow?.github && team.github_repo) {
      await removeRepoCollaborator(team.github_repo, memberRow.github).catch((e) =>
        logger.error(`GitHub remove collaborator failed: ${e instanceof Error ? e.message : String(e)}`)
      );
    }

    await interaction.editReply(`✅ Removed ${user} from **${teamSlug}**.`);
    await maybeAudit(interaction, `➖ ${interaction.user.tag} force-removed ${user.tag} from ${teamSlug}.`);
    return;
  }
  ...
  if (sub === "force_transfer") {
    ...
    if (currentLeader && currentLeader !== user.id) {
      await db.updateTeamMember(teamSlug, currentLeader, TeamPermissionLevel.MEMBER).catch(() => {});
    }
    ...
    await interaction.editReply(`✅ Leadership of **${teamSlug}** transferred to ${user}.`);
    await maybeAudit(interaction, `👑 ${interaction.user.tag} force-transferred ${teamSlug} leadership to ${user.tag}.`);
    return;
  }
