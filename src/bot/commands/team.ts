  ...
  try {
    ...
    if (!team) {
      await interaction.editReply(
        "You're not on a team yet, and this channel isn't linked to one."
      );
      return;
    }
    ...
    logger.info(
      `Team overview fetched for ${team.slug} by ${interaction.user.tag}`
    );
    await interaction.editReply({ embeds: [embed] });
  } catch (err) {
    logger.error(
      `Failed to get team info for ${interaction.user.tag}: ${err instanceof Error ? err.message : String(err)}`
    );
