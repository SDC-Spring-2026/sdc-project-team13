...
export async function handleDisable(interaction: ChatInputCommandInteraction) {
  ...
  if (!isGuildAdmin(interaction)) {
    await interaction.reply({
      flags: MessageFlags.Ephemeral,
      content: "Only server administrators can disable teams."
    });
    return;
  }
  ...
  if (team.is_disabled) {
    await interaction.reply({
      flags: MessageFlags.Ephemeral,
      content: `**${group}** is already disabled.`
    });
    return;
  }
  ...
  await db.setTeamDisabled(teamSlug, true);
  ...
  if (team.github_repo) {
    await setTeamRepoArchived(team.github_repo, true).catch((e) => {
      logger.error(
        `Failed to archive repo for "${teamSlug}": ${e instanceof Error ? e.message : String(e)}`
      );
  ...
  await interaction.editReply(
    `✅ **${group}** has been disabled. The channel is now hidden and the repo archived.${githubNote}`
  );
}
