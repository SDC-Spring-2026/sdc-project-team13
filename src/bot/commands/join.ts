...
export async function handleJoin(interaction: ChatInputCommandInteraction) {
  ...
  if (!teamSlug) {
    await interaction.reply({
      flags: MessageFlags.Ephemeral,
      content: `No group called **${name}** found!`
    });
    return;
  }

  const existingPerm = await db.getMemberTeamPermission(
    teamSlug,
    interaction.user.id
  );
  if (existingPerm !== null) {
    await interaction.reply({
      flags: MessageFlags.Ephemeral,
      content: `You are already a member of **${name}**!`
    });
    return;
  }
  ...
  if (leaderIds.length === 0) {
    await interaction.reply({
      flags: MessageFlags.Ephemeral,
      content: `Could not find any leaders for **${name}**!`
    });
    return;
  }
  ...
  for (const leaderId of leaderIds) {
    try {
      ...
      logger.warn(
        `Failed to DM leader ${leaderId} for join request to "${name}": ${e instanceof Error ? e.message : String(e)}`
      );
    }
  }
  ...
  if (sentDMs.length === 0) {
    await interaction.reply({
      flags: MessageFlags.Ephemeral,
      content: `Could not reach any leaders for **${name}**. Try contacting them directly.`
    });
    return;
  }

  logger.info(
    `${interaction.user.tag} (${interaction.user.id}) sent join request to "${name}" (team: ${teamSlug}), DMed ${sentDMs.length} leader(s)`
  );
  await interaction.reply({
    flags: MessageFlags.Ephemeral,
    content: `✅ Join request sent to **${name}**!`
  });
  ...
  for (const { message } of sentDMs) {
    ...
    collector.on("collect", async (buttonInteraction) => {
      ...
      if (buttonInteraction.customId === "join_accept") {
        try {
          ...
          if (joiningMember?.github && team?.github_repo) {
            await addRepoCollaborator(
              team.github_repo,
              joiningMember.github
            ).catch((e) =>
              logger.error(
                `Failed to add GitHub collaborator for ${interaction.user.tag}: ${e instanceof Error ? e.message : String(e)}`
              )
            );
          }

          logger.info(
            `${interaction.user.tag} accepted into "${name}" (team: ${teamSlug}) by ${buttonInteraction.user.tag}`
          );
          ...
          logger.error(
            `Failed to add ${interaction.user.tag} to "${name}": ${err instanceof Error ? err.message : String(err)}`
          );
          await buttonInteraction.update({
        ...
        logger.info(
          `${interaction.user.tag}'s request to join "${name}" declined by ${buttonInteraction.user.tag}`
        );
    ...
    collector.on("end", async (collected) => {
      if (!handled && collected.size === 0) {
        logger.warn(
          `Join request from ${interaction.user.tag} to "${name}" expired with no response`
        );
        await message
          .edit({
