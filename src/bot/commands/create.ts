...
export async function handleCreate(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    if (!category) {
      logger.warn(
        `Teams category not found in guild ${guild.id} — rolling back`
      );
    ...
    logger.info(
      `Project "${project}" created by ${interaction.user.tag} (team: ${teamSlug}, channel: ${channel.id}, role: ${role.id})`
    );
  } catch (err) {
    logger.error(
      `Failed to create project "${project}" for ${interaction.user.tag}: ${err instanceof Error ? err.message : String(err)}`
    );
    if (channel)
      await channel
        .delete()
        .catch((e) =>
          logger.error(
            `Channel rollback failed: ${e instanceof Error ? e.message : String(e)}`
          )
        );
    if (role)
      await role
        ...
        .catch((e) =>
          logger.error(
            `Role rollback failed: ${e instanceof Error ? e.message : String(e)}`
          )
        );
    if (teamSlug)
      await db
        ...
        .catch((e) =>
          logger.error(
            `Team record rollback failed: ${e instanceof Error ? e.message : String(e)}`
          )
        );
    await interaction.editReply(
      "Failed to create project. Check bot permissions and try again."
    );
    return;
  }
  ...
  try {
    ...
    logger.error(
      `GitHub setup failed for team "${teamSlug}": ${err instanceof Error ? err.message : String(err)}`
    );
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Role", value: `${role}`, inline: true },
      {
        ...
        value: repoUrl
          ? `[${teamSlug}](${repoUrl})`
          : "⚠️ Repo setup failed — contact an admin."
      }
    )
    .setTimestamp();
  ...
  await welcomeMsg
    ...
    .catch((e) =>
      logger.warn(
        `Failed to pin welcome embed in ${channel.id}: ${e instanceof Error ? e.message : String(e)}`
      )
    );
