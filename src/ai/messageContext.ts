...
async function buildAiSessionContextInner(message: Message): Promise<string> {
  ...
  if (!message.guild) {
    ...
    if (teams.length > 0) {
      lines.push(`User's active DB team slugs: ${teams.map((t) => t.slug).join(", ")}`);
    }
  ...
  const chName = ch.isDMBased() ? "dm" : "name" in ch ? ch.name ?? "unknown" : "unknown";
  lines.push(`Channel: #${chName} (${message.channelId})${ch.isThread() ? " (thread)" : ""}`);
  ...
  if (member) {
    ...
    lines.push(`User: ${author.tag} (${author.id}), display name: ${member.displayName}`);
    lines.push(
  ...
  if (team) {
    ...
    lines.push(`User's role on this team in the DB: ${formatDbTeamRole(perm)}.`);

    const projects = await db.getTeamProjects(team.slug);
    const activeNames = projects.filter((p) => p.is_active).map((p) => p.name);
    if (activeNames.length > 0) {
      lines.push(`Active project names on this team: ${activeNames.slice(0, 20).join("; ")}`);
    } else {
    ...
    lines.push("Roster with names from this Discord server (use these names when the user asks who is on the team):");
    for (const m of roster.slice(0, 50)) {
      const p = await db.getMemberTeamPermission(team.slug, m.discord);
      const roleLabel =
        p === TeamPermissionLevel.LEADER ? "leader" : "member";
      const label = labels.get(m.discord) ?? `Discord id ${m.discord}`;
      lines.push(`- ${label} — ${roleLabel}`);
    }

    const recent = await db.getRecentTeamMessages(team.slug, RECENT_MESSAGE_HISTORY);
  ...
  if (userTeams.length > 0) {
    lines.push(`User's active DB team slugs: ${userTeams.map((t) => t.slug).join(", ")}`);
  }
